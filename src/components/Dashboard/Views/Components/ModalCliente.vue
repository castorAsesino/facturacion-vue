...
export default {
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          const operacion = this.clienteId ? 'editarCliente' : 'crearCliente';
          this.loadingGuardadoCliente = true;
          axios.post(`/Personas/${operacion}`, this.ruleForm)
            .then((response) => {
              if (!response.data.error) {
  ...
  watch: {
    clienteId(newValue, oldValue) {
      if (newValue) {
        axios.get('/Personas/listarClientes', { params: { id: newValue } })
          .then(response => {
            if (!response.data.error) {
              const data = response.data.dato[0];
              this.ruleForm = {
                id: data.id,
                nombre: data.nombre,
                codigoLegajo: data.codigoLegajo,
